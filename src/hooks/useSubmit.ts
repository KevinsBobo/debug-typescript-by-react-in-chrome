...
import { AccountLayout, approveChecked, createMint, getAccount, getMint, getOrCreateAssociatedTokenAccount, mintTo, TOKEN_PROGRAM_ID, transfer } from '@solana/spl-token'; // @FIXME: replace with @solana/spl-token
...
async function checkApproved(account: PublicKey) {
    ...
    console.log(approvedAccountInfo.delegate)
...
async function createMintAndTransferTokens() {
    ...
    const mint = await createMint(connection, fromWallet, mintAuthority.publicKey, freezeAuthority.publicKey, 9);
